...
namespace Chained {
    ...
    {
        // ====== HOTKEY TO OPEN DEBUG EDITOR STATE ======
        static bool editorKeyHeld = false;
        int hState = glfwGetKey(engine->getWindow(), GLFW_KEY_H);
        if (hState == GLFW_PRESS && !editorKeyHeld) {
            engine->run(std::make_unique<DebugEditorState>(engine));
            editorKeyHeld = true;
            return; // Make sure you return so MainMenu doesn't keep running!
        }
        if (hState == GLFW_RELEASE) {
            editorKeyHeld = false;
        }
        // ===============================================
